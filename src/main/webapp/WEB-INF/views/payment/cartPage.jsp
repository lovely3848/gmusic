...
#cartlist:focus, #lyrics:focus, #audioplay:focus {
	outline: none;
}
...
<script>
	...
	function autoplay() {
		$("#cartlist option:first").prop("selected", true); // 노래 리스트의 첫곡을 선택 (select의 상태를 true로 활성화)
		nowplay(); // 현재곡 재생함수
	...
	function previous() {
		...
		var index = $("#cartlist option").index($("#cartlist option:selected")); // 옵션중에서 리스트 옵션이 select된 항목의 index를 변수에 저장.

		index = index - 1; // 이전곡 = 현재곡의 인덱스 - 1
		$("#cartlist option:eq(" + index + ")").prop("selected", true); // 선택. 리스트의 옵션에서 eq안의 인덱스에 해당하는 옵션의 select 상태를 true로 만들어준다.
	...
	function nowplay() { // 현재 선택곡 재생. 각 div들에 현재 재생할 값들을 넣어서 표현해줌
		...
		$('#nowCheck').val($("#cartlist option:selected").attr('id'));

		document.title = $("#cartlist option:selected").attr('value') + " - "
				+ $("#cartlist option:selected").attr('value2'); // 현재 곡에 따라 타이틀 변경
		if ($("#cartlist option:selected").attr('value') != null
				&& $("#cartlist option:selected").attr('value') > 0) {
		}

		$("#sname").html($("#cartlist option:selected").attr('value'));
		$("#singername").html($("#cartlist option:selected").attr('value2'));
		$("#albumimage").attr("src",
				$("#cartlist option:selected").attr('value3'));
		$("#audioplay").attr("src",
				$("#cartlist option:selected").attr('value4'));

		if ($("#cartlist option:selected").attr('value5') != "") {
			$("#lyrics").html($("#cartlist option:selected").attr('value5'));
		} else {
			$("#lyrics").html("가사가 없습니다");
		}

		$("#albumimageORlyrics").html(
				$("#cartlist option:selected").attr('value5'));
		var snumber = $("#cartlist option:selected").attr('value6');
		// 볼륨조절
	...
	function next() {
		...
		var index = $("#cartlist option").index($("#cartlist option:selected")); // 리스트 옵션에서 현재 선택된 노래의 인덱스를 저장.
		var lastindex = $("#cartlist option:last").prop("selected"); // true or false  // 마지막 노래가 선택되었는지를 변수에 저장. 참고: (prop("selected")는 boolean타입이다.)

		if (lastindex) { // 마지막 노래에서 다음곡을 누른 사실이 true이면
			autoplay(); // 리스트의 첫 곡을 실행한다.
		} else { // 나머지 경우라면  
			index = index + 1; // 현재 선택된 노래의 인덱스의 다음인덱스. 미래인덱스= 현재인덱스+1
			$("#cartlist option:eq(" + index + ")").prop("selected", true); // 미래인덱스 선택. 리스트의 옵션에서 eq안의 인덱스에 해당하는 옵션의 select 상태를 true로 만들어준다.
			nowplay(); // 선택된 곡 재생
		}
	...
	function shuffle() { // 반복재생
		location.href = 'cartlist?cartVal=' + $('#cartValss').val()
				+ '&jcode=U';
	} //shuffle
	...
	var selectbox = {
		...
		remove : function(obj) { // cartlist.   obj는 cartlist(선택한 노래 목록)이다.
			var index = $("#cartlist option").index(
					$("#cartlist option:selected"));
			var cartValSession = $('#cartValss').val();
			var cartValsplit = cartValSession.split(',');
			var cartVal = '';
			var nowplayId = $('#nowCheck').val();
			var selectedId = $("#cartlist option:selected").attr('id');

			cartValsplit[index] = '';

			this.ajax(cartValsplit, cartVal);

			for (var k = obj.options.length - 1; k >= 0; k--) { // cartlist의 마지막인덱스 부터 0번쨰 인덱스 까지. 역순인 이유는 중간에 노래가 삭제되면 마지막 인덱스 k에 해당하는 옵션이 없어지므로 조건절의 오류가 나기 떄문이다.
				if (obj.options[k].selected) { // cartlist의 k번째 옵션이 selected(==true)라면
					obj.options[k] = null; // 선택되어있는 곡을 cartlist에서 지운다.
				}
			}
		...
		moveUp : function(obj) { // cartlist
			...
			if (index != 0) {
				tmp = cartValsplit[index];
				cartValsplit[index] = cartValsplit[index - 1];
				cartValsplit[index - 1] = tmp;
			}
			...
			for (var k = 1; k < obj.options.length; k++) { // 0번째(첫번쨰 곡)의 윗 곡는 없다. 1번째부터 마지막 옵션까지.
				if (obj.options[k].selected) { // 반복하는 중 k번째 노래가 현재 선택되어있는 노래고,
					this.swapObj(obj, k, k - 1); // 밑의 코드를 실행. cartlist(목록)의 현재곡과 이전곡의 옵션(순서)을 바꾼다.
				}
			}
		},
		...
		moveDown : function(obj) { // 마지막곡의 다음곡은 없다. 마지막곡의 이전곡부터 첫번쨰 곡까지.
			...
			var cartVal = '';
			var tmp = '';
			var lastindex = $("#cartlist option:last").prop("selected");
			...
			for (var k = obj.options.length - 2; k >= 0; k--) { // 마지막 인덱스(마지막 노래)의 이전곡부터 첫번쨰 곡까지 역순. (정렬 알고리즘의 원래와 비슷함. 스위치된 k+1번째 값이 확인되지 않고 건너뛰는 경우를 막기위함.)
				if (obj.options[k].selected) { // 반복하는 중 k번째 노래가 현재 선택되어있는 노래고,
					this.swapObj(obj, k, k + 1); // 밑의 코드를 실행. cartlist(목록)의 현재곡과 다음곡의 옵션(순서)을 바꾼다.
				}
			}
		},

		/* 위치를 서로 바꾸는 함수 */
		swapObj : function(obj, i, j) { // funtion(cartlist, k, k+1) or funtion(cartlist, k, k-1)
		...
		ajax : function(cartValsplit, cartVal) {
			...
			var url = '/green/cartlist?cartVal=' + $('#cartValss').val();
...
<body onload="autoplay()">
	...
	<div class="layer">
		<span class="content">
			<div id="cartlistDiv">
				<table id="cartlistTable">
					...
					<c:forEach var="row" items="${Banana}" varStatus="vs">
						<tr>
							<td>${vs.count}
								<input type="hidden" value="${row.snum}">
							</td>
							<td align="left">${row.sname}&nbsp;-&nbsp;${row.singername}</td>
							<td>300원</td>
							<td>
			...
			<button type="button" onclick="">개별 곡 결제하기</button>
		</span>
	</div>
</body>
</html>